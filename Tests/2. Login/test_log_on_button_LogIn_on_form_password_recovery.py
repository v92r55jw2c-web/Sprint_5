...
from locators import (
    ...
    BUTTON_LOGIN_RECOVERY_PASSWORD,
    EMAIL_LOG_IN,
    PASSWORD_LOG_IN,
    BUTTON_LOGIN,
    BUTTON_ORDER
...
def test_log_on_button_LogIn_on_form_password_recovery(driver):
    driver.get("https://stellarburgers.education-services.ru/")

    WebDriverWait(driver, 10).until(expected_conditions.element_to_be_clickable(BUTTON_PERSONAL_ACCOUNT)).click()

    driver.find_element(*BUTTON_RECOVERY_PASSWORD).click()

    driver.find_element(*BUTTON_LOGIN_RECOVERY_PASSWORD).click()
